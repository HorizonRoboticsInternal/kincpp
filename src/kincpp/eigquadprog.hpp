...
// This is an Eigen version of https://github.com/liuq/QuadProgpp
...
inline double solve_quadprog(MatrixXd& G, VectorXd& g0, const MatrixXd& CE, const VectorXd& ce0,
...
l2a: /* Step 2a: determine step direction */
    ...
    l = 0;
    /* Compute t1: partial step length (maximum step in dual space without violating dual
     * feasibility */
    t1 = inf; /* +inf */
    ...
    /* Compute t2: full step length (minimum step in primal space such that the constraint ip
     * becomes feasible */
    if (abs(z.dot(z)) > std::numeric_limits<double>::epsilon())  // i.e. z != 0
        t2 = -s(ip) / z.dot(np);
    else
...
inline bool add_constraint(MatrixXd& R, MatrixXd& J, VectorXd& d, int& iq, double& R_norm) {
    ...
    for (j = n - 1; j >= iq + 1; j--) {
        /* The Givens rotation is done with the matrix (cc cs, cs -cc).
                             ...
                             If cc is zero, then we just have to switch column (j) and column (j -
           1) of J. Since we only switch columns in J, we have to be careful how we update d
           depending on the sign of gs. Otherwise we have to apply the Givens rotation to these
...
inline void delete_constraint(MatrixXd& R, MatrixXd& J, VectorXi& A, VectorXd& u, int p, int& iq,
                              int l) {
